...
export default function UserProfilePage() {
  ...
  const updateEmailMutation = useMutation({
    mutationFn: async (email: string) => {
      ...
      const response = await fetch(`/api/app-users/${userQuery.data.id}`, {
        method: "PATCH",
        headers: {
  ...
  const updatePasswordMutation = useMutation({
    mutationFn: async ({ currentPassword, newPassword }: { currentPassword: string, newPassword: string }) => {
      ...
      const response = await fetch(`/api/users/${userQuery.data.jellyfinUserId}/password`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  return (
    <motion.div 
      className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-900 via-gray-900 to-black text-white overflow-hidden"
      initial="hidden"
      animate="visible"
    ...
    >
      ...
      <div className="fixed inset-0 z-0">
        ...
        {/* Cinematic light ray effect */}
        <motion.div 
          className="absolute top-0 -right-10 w-[500px] h-[800px] bg-blue-700/20 blur-[120px] rounded-full"
          animate={{ 
            opacity: [0.1, 0.2, 0.1],
            x: [0, -20, 0]
          }}
        ...
        <motion.div 
          className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-indigo-900/20 blur-[100px] rounded-full"
          animate={{ 
      ...
      <header className="bg-black/40 backdrop-blur-xl border-b border-white/10 text-white sticky top-0 z-50 shadow-lg shadow-blue-900/20">
        <div className="container mx-auto px-4 py-3 flex flex-wrap items-center justify-between relative">
          ...
          >
            ...
            >
              {getServerLogo() ? (
                ...
                <div className="h-10 w-10 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-md flex items-center justify-center text-white shadow-lg shadow-blue-700/20">
            ...
            <div>
              <motion.h1 
                className="text-xl font-medium leading-none bg-clip-text text-transparent bg-gradient-to-r from-blue-300 to-white"
                whileHover={{ scale: 1.02 }}
                transition={{ type: "spring", stiffness: 500 }}
              >
                {getServerName()}
              </motion.h1>
              <p className="text-xs text-blue-300/80">User Dashboard</p>
            </div>
          </motion.div>
          ...
          >
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button 
                variant="outline" 
                size="sm"
                className="border-white/20 text-white hover:bg-white/10 hover:border-blue-400 transition-all duration-300"
          ...
          <motion.div 
            className="absolute bottom-0 left-0 h-[1px] bg-gradient-to-r from-transparent via-blue-500 to-transparent w-full"
            animate={{ 
              opacity: [0.3, 0.6, 0.3],
        ...
        {userQuery.isLoading ? (
          ...
          >
            ...
            <motion.div variants={itemVariants}>
              <Card className="md:col-span-1 bg-gradient-to-b from-gray-900 to-gray-950 border border-blue-900/40 text-white overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-blue-800/30 rounded-xl backdrop-blur-sm relative">
                {/* Decorative elements for cinematic feel */}
                <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-800 via-indigo-600 to-blue-800 opacity-80"></div>
                <div className="absolute -left-20 top-10 w-40 h-60 bg-blue-600/20 blur-[60px] rounded-full"></div>
                <div className="absolute -right-20 bottom-10 w-40 h-60 bg-indigo-600/20 blur-[60px] rounded-full"></div>
                ...
                <CardHeader className="pb-0 pt-8 px-6 flex flex-col items-center relative">
                  ...
                  >
                    <div className="absolute inset-0 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 blur-md opacity-70 scale-110"></div>
                    <Avatar className="h-28 w-28 bg-gray-800 ring-4 ring-blue-500/30 relative">
                ...
                <CardFooter className="bg-gradient-to-b from-gray-800/80 to-gray-900 p-6 border-t border-blue-900/30 relative">
                  ...
                  {/* Decorative lens flare for button */}
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 w-20 h-[1px] bg-blue-400/50 blur-sm"></div>
                </CardFooter>
              </Card>
            </motion.div>
                    ...
                    <div className="space-y-4 rounded-xl overflow-hidden">
                      <AnimatePresence>
                        {watchHistoryQuery.data.Items.map((activity, idx) => (
                          <motion.div 
                            key={idx} 
                            className="flex items-start border-b border-blue-900/30 pb-4 last:border-0 last:pb-0 hover:bg-blue-900/10 p-3 rounded-lg transition-all duration-300 relative"
